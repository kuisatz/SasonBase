...
namespace SasonBase.Reports.Sason.Merkez
{
    ...
    /// </summary>
    public class MrkzYdkPrcFlytRpr2 : Base.SasonMerkezReporter
    {
        public MrkzYdkPrcFlytRpr2()
        {
            Text = "Yedek Parça Faliyet Raporu hatalı sadece yedek olarak  tutuluyor";  
        ...
        public MrkzYdkPrcFlytRpr2(decimal servisId, DateTime startDate, DateTime finishDate) : this()
        {
        ...
        public override object ExecuteReport(MethodReturn refMr = null)
        {
            ...
            List<QueryResult> queryResults = AppPool.EbaTestConnector.CreateQuery($@"
             ...
             group by SERVISSTOKTURID,BELGETURU,AYRISTIRMATIPAD,ISEMIRTIPI
             ) ert  
                ")
             .GetDataTable(mr)
            ...
            reportData.queryr = queryResults;
            ...
            List<QueryResultStok> queryStok = AppPool.EbaTestConnector.CreateQuery($@"
                   select    '{YEDEKPARCATOPLAM1}'    as YEDEKPARCATOPLAM,
                        ...
                        sum(nvl(stok_oes,0) +nvl(stok_oeM,0) + nvl(stok_essanayi,0) + nvl(stok_my,0) +nvl(stok_yansanayi,0)) as stok_toplam
                      FROM (
                      SELECT   
                                case SERVISSTOKTURID 
                                        when 1 then  sum(fiyat)  
                                     end   as stok_oes,
                                case SERVISSTOKTURID  
                                        when 7 then  sum(fiyat)                               
                                     end   as stok_oeM,  
                                case SERVISSTOKTURID  
                                        when 8 then  sum(fiyat)  
                                     end   as stok_essanayi, 
                                case SERVISSTOKTURID  
                                        when 9 then  sum(fiyat)  
                                     end   as stok_yansanayi,              
                                case SERVISSTOKTURID  
                                        when 11 then  sum(fiyat) 
                                     end   as stok_my
