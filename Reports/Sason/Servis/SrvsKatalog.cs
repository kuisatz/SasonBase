...
namespace SasonBase.Reports.Sason.Servis
{
    ...
    ///   Katalok raporu
    /// </summary>
    public class SrvsKatalog : Base.SasonReporter
    {
        public SrvsKatalog()
        {
            Text = "Agrega Kataloğu";
            SubjectCode = "SrvsKatalog";
            SubjectCode = this.getType().Name;
            ReportFileCode = this.getType().Name;
            AddParameter(new ReporterParameter() { Name = "param_agreta", Text = "Agrega" }.CreateAgregaSelect(true));
            Disabled = false;
        }
        public SrvsKatalog(decimal servisId, decimal agretaId, DateTime startDate, DateTime finishDate) : this()
        {
